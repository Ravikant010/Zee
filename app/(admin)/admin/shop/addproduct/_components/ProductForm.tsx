...
import DBProduct, { DBProductImages } from "../actions/product";
import { useContext } from "react";

import { useRouter } from "next/navigation";
import { ShopInterfaceContext, useShopContextFunction } from "../../layout";
import { ShopIdContextType, useContextFunction } from "../layout";
import { ShoppingBasketIcon } from "lucide-react";
function ProductForm() {
  const [category, setCategory] = useState("");
  const [formPhase, setFormPhase] = useState(1);
  const [selectedSizes, setSelectedSizes] = useState<string[]>([]);
  const {handleSetProductId, product_id, image_upload_status} = useContextFunction() as ShopIdContextType
  
  const {shop} = useShopContextFunction()  as ShopInterfaceContext
 ...
 async function onFinalSubmit(){
    ...
    const payload = {...form_one, ...from_two, product_sizes: [...selectedSizes], shop_id: shop?.id, product_category:category}
  ...
  return (
    <main>
      <Heading shop_name={shop?.shop_name as string}/>
      <Form {...productFormOne}>
        <form
